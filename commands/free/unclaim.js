...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!ticket.claimedBy) {
            const errorEmbed = createEmbed('Error', 'This ticket is not claimed.', 0xFF0000);
            return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }

        if (ticket.claimedBy !== interaction.user.id && !interaction.member.permissions.has('ADMINISTRATOR')) {
            const errorEmbed = createEmbed('Error', `You cannot unclaim a ticket claimed by <@${ticket.claimedBy}>.`, 0xFF0000);
            return interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
        ...
        const successEmbed = createEmbed('Ticket Unclaimed', `This ticket has been unclaimed.`);
